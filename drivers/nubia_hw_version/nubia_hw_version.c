...
int nubia_gpio_ctrl1(struct platform_device *pdev)
{
	...
	rc = pinctrl_select_state(gpio_data->pinctrl,
				  gpio_data->pull_down_default);
	return rc;
}
...
int nubia_gpio_ctrl(struct platform_device *pdev)
{
	...
	gpio_data =
	    devm_kzalloc(&pdev->dev, sizeof(struct nubia_gpio_pinctrl_info),
			 GFP_KERNEL);
	if (!gpio_data)
		return -ENOMEM;
	...
	gpio_data->pull_up_default =
	    pinctrl_lookup_state(gpio_data->pinctrl, "pull_up_default");
	...
	gpio_data->pull_down_default =
	    pinctrl_lookup_state(gpio_data->pinctrl, "pull_down_default");
	...
	ret =
	    pinctrl_select_state(gpio_data->pinctrl,
				 gpio_data->pull_up_default);
	if (ret) {
...
//开始进入第一大部分-读取pcb版本部分
#ifdef CONFIG_NUBIA_HW_VER_BY_ADC
const struct hw_pcb_adc_map_str *nubia_get_pcb_table_item_by_adc(const struct
								 hw_pcb_adc_map_str
								 *pts,
								 uint32_t
								 tablesize,
...
#else
const struct hw_pcb_gpio_map_str *nubia_get_pcb_table_item_by_gpio(const struct
								   hw_pcb_gpio_map_str
								   *pts,
								   uint32_t
								   tablesize)
...
//开始读取第三大部分-软件相关的一些配置
#ifdef CONFIG_NUBIA_HW_CONFIG_BY_GPIO
const struct hw_config_gpio_map_st *nubia_get_config_table_item_by_gpio(const
									struct
									hw_config_gpio_map_st
									*pts,
									uint32_t
									tablesize)
{
...
#else
const struct hw_config_adc_map_st *nubia_get_config_table_item_by_adc(const
								      struct
								      hw_config_adc_map_st
								      *pts,
								      uint32_t
								      tablesize,
								      int input)
...
#ifdef CONFIG_NUBIA_HW_VER_BY_ADC
#else
static int nubia_parse_hw_ver_gpio_dt(struct device_node *node,
				      int *pcb_gpio1_v, int *pcb_gpio2_v)
{
	int pcb_gpio1 = 0;
	int pcb_gpio2 = 0;
	int rc = 0;
	...
	if (rc < 0) {
		printk("Failed to request GPIO:%d, ERRNO:%d", (s32) pcb_gpio1,
		       rc);
		rc = -ENODEV;
	}
...
#endif
#ifdef CONFIG_NUBIA_HW_GPIO_BY_PM
static int nubia_parse_hw_ver_pm_gpio_dt(struct device_node *node,
					 int *pm_gpio1_v, int *pm_gpio2_v)
{
	int pm_gpio1 = 0;
	int pm_gpio2 = 0;
	int rc = 0;
...
#endif
static int nubia_parse_hw_ver_rf_gpio_dt(struct device_node *node,
					 int *rf_gpio1_v, int *rf_gpio2_v)
{
	int rf_gpio1 = 0;
	int rf_gpio2 = 0;
	int rc = 0;
	...
	if (rc < 0) {
		printk("Failed to request GPIO:%d, ERRNO:%d", (s32) rf_gpio1,
		       rc);
		rc = -ENODEV;
	}
...
#ifdef CONFIG_NUBIA_HW_CONFIG_BY_GPIO
static int nubia_parse_hw_ver_config_gpio_dt(struct device_node *node,
					     int *config_gpio1_v,
					     int *config_gpio2_v)
{
	int config_gpio1 = 0;
...
#ifdef CONFIG_NUBIA_HW_VER_BY_ADC
static int nubia_parse_hw_ver_adc_dt(struct device_node *node,
				     int *pcb_adc_channel_v)
{
	int pm_adc_channel = 0;
	int rc = 0;

	rc = of_property_read_u32(node, "qcom,pcb-pm-adc_channel",
				  &pm_adc_channel);
...
#ifdef CONFIG_NUBIA_HW_CONFIG_BY_GPIO
#else
static int nubia_parse_hw_ver_config_adc_dt(struct device_node *node,
					    int *config_pm_adc_channel_v)
{
	int pm_adc_channel = 0;
	int rc = 0;

	rc = of_property_read_u32(node, "qcom,config-pm-adc_channel",
				  &pm_adc_channel);
...
static int nubia_parse_hw_ver_read_adc(int pm_adc_channel_v, int *result)
{
	...
	if (penv->vadc_dev != NULL) {
		rc = qpnp_vadc_read(penv->vadc_dev, pm_adc_channel_v,
				    &adc_result);
		if (rc) {
...
static int nubia_hw_ver_probe(struct platform_device *pdev)
{
	...
	if (rc < 0)
		return rc;
#ifdef CONFIG_NUBIA_HW_CONFIG_BY_GPIO
	rc = nubia_parse_hw_ver_config_gpio_dt(node, &config_gpio1,
					       &config_gpio2);
	if (rc < 0)
		return rc;
#else
...
//if use pm gpio
	...
	nubia_config_gpio1_v += nubia_get_gpio_status(config_gpio1);
	nubia_config_gpio2_v += nubia_get_gpio_status(config_gpio2);
	nubia_hw_version_debug
	    ("nubia_config_gpio1_v=%x,nubia_config_gpio2_v=%x\n",
...
int __init nubia_hw_version_init(void)
{
	...
	if (rc)
		printk(KERN_ERR
		       "%s: failed to create nubia_hw_version group attributes\n",
		       __func__);
